...
class Language
{
    ...
    public static function tradition2simplified($tc) {
        $iContent = mb_strlen($tc, 'UTF-8');
        $simplifiedCN = "";
        for ($i = 0; $i < $iContent; $i++) {
            $str = mb_substr($tc, $i, 1, 'UTF-8');
            $match = mb_strpos(self::$td, $str, null, 'UTF-8');
            $simplifiedCN.=($match !== false ) ? mb_substr(self::$sd, $match, 1, 'UTF-8') : $str;
        }
        return $simplifiedCN;
    }

    public static function simplified2tradition($sc) {
